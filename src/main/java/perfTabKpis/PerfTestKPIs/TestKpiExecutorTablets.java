package perfTabKpis.PerfTestKPIs;
...
import perfTabKpis.LatencyTestTab.LatencyTest;

import java.io.IOException;

@Listeners(testReporter.TestAdapter.class)
public class TestKpiExecutorTablets extends CommonTools {
     ...
     */
    @Test(priority = 1, groups = {"latency", "cool"})
    public void Cool_Latency_FirstFrame() throws IOException {
        latencyTest.run_LatencyTest(DSN, "Cool_FF");
     ...
     * @throws IOException if an I/O exception occurs while running the test.
     */
    @Test(priority = 2, groups = {"latency", "warm"})
    public void Warm_Latency_FirstFrame() throws IOException {
        latencyTest.run_LatencyTest(DSN, "Warm_FF");
        forceStopApp(DSN, getAppPackage(DSN));
    }
